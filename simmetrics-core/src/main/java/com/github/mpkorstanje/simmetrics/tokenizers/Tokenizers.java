...
public final class Tokenizers {
     ...
     *
     * @param pattern to split the the string around
     ...
     *
     * @param regex to split the the string around
     * @return a pattern tokenizer
     */
     ...
     *
     * @param q       size of the tokens
     * @param padding padding to pad start and end of string with
     * @return a q-gram tokenizer
     */
     ...
     *
     * @param q            size of the tokens
     * @param startPadding padding to pad start of string with
     * @param endPadding   padding to pad end of string with
     * @return a q-gram tokenizer
     */
     ...
     *
     * @param tokenizer delegate tokenizer
     * @param function  to transform tokens
     ...
     *
     * @param tokenizers a non-empty list of tokenizers
     ...
     * If only a single tokenizer is provided, that tokenizer is returned.
     *
     * @param tokenizer  the first tokenizer
     * @param tokenizers a the other tokenizers
     * @return a chain of tokenizers
     */
     ...
     *
     * @param tokenizer delegate tokenizer
     * @param predicate for tokens to keep
    ...
    static class Filter implements Tokenizer {
        ...
        static final class TransformFilter extends
            ...
            @Override
            Stream<String> tokenizeToFilteredList(String input) {
                return tokenizer.tokenizeToTransformedList(input).filter(predicate);
            }

            @Override
            Stream<String> tokenizeToFilteredMultiset(String input) {
                return tokenizer.tokenizeToTransformedMultiset(input).filter(predicate);
            }

            @Override
            Stream<String> tokenizeToFilteredSet(String input) {
                return tokenizer.tokenizeToTransformedSet(input).filter(predicate);
            }

            @Override
            public List<String> tokenizeToList(String input) {
                return tokenizer.tokenizeToTransformedList(input).filter(predicate).collect(toList());
            }

            @Override
            public Multiset<String> tokenizeToMultiset(String input) {
                return tokenizer.tokenizeToTransformedMultiset(input).filter(predicate).collect(toMultiset());
            }

            @Override
            public Set<String> tokenizeToSet(String input) {
                return tokenizer.tokenizeToTransformedList(input).filter(predicate).collect(toSet());
            }
        ...
        static Tokenizer createCombined(Filter tokenizer,
            ...
            if (tokenizer instanceof TransformFilter) {
                ...
                return new TransformFilter(tft.getTokenizer(),
                        tft.getPredicate().and(predicate));
            }

            return new Filter(tokenizer.getTokenizer(),
                    tokenizer.getPredicate().and(predicate));
        }
        ...
        Stream<String> tokenizeToFilteredList(String input) {
            return tokenizer.tokenizeToList(input).stream().filter(predicate);
        }

        Stream<String> tokenizeToFilteredMultiset(String input) {
            return tokenizer.tokenizeToMultiset(input).stream().filter(predicate);
        }

        Stream<String> tokenizeToFilteredSet(String input) {
            return tokenizer.tokenizeToSet(input).stream().filter(predicate);
        }

        @Override
        public List<String> tokenizeToList(String input) {
            return tokenizer.tokenizeToList(input).stream().filter(predicate).collect(toList());
        }

        @Override
        public Multiset<String> tokenizeToMultiset(String input) {
            return tokenizer.tokenizeToMultiset(input).stream().filter(predicate).collect(toMultiset());
        }

        @Override
        public Set<String> tokenizeToSet(String input) {
            return tokenizer.tokenizeToSet(input).stream().filter(predicate).collect(toSet());
        }
    ...
    static final class Split implements Tokenizer {
    ...
    static class Transform implements Tokenizer {
        ...
        static final class FilterTransform extends
            ...
            @Override
            public List<String> tokenizeToList(String input) {
                return tokenizer.tokenizeToFilteredList(input).map(function).collect(toList());
            }

            @Override
            public Multiset<String> tokenizeToMultiset(String input) {
                return tokenizer.tokenizeToFilteredMultiset(input).map(function).collect(toMultiset());
            }

            @Override
            public Set<String> tokenizeToSet(String input) {
                return tokenizer.tokenizeToFilteredSet(input).map(function).collect(toSet());
            }

            @Override
            Stream<String> tokenizeToTransformedList(String input) {
                return tokenizer.tokenizeToFilteredList(input).map(function);
            }

            @Override
            Stream<String> tokenizeToTransformedMultiset(String input) {
                return tokenizer.tokenizeToFilteredMultiset(input).map(function);
            }

            @Override
            Stream<String> tokenizeToTransformedSet(String input) {
                return tokenizer.tokenizeToFilteredSet(input).map(function);
            }
        ...
        static Tokenizer createCombined(Transform tokenizer,
            ...
            if (tokenizer instanceof FilterTransform) {
                ...
                return new FilterTransform(ftt.getTokenizer(), tokenizer.getFunction().andThen(function));
            }

            return new Transform(tokenizer.getTokenizer(), tokenizer.getFunction().andThen(function));
        }
        ...
        @Override
        public List<String> tokenizeToList(String input) {
            return tokenizer.tokenizeToList(input).stream().map(function).collect(toList());
        }

        @Override
        public Multiset<String> tokenizeToMultiset(String input) {
            return tokenizer.tokenizeToMultiset(input).stream().map(function).collect(toMultiset());
        }

        @Override
        public Set<String> tokenizeToSet(String input) {
            return tokenizer.tokenizeToSet(input).stream().map(function).collect(toSet());

        }

        Stream<String> tokenizeToTransformedList(String input) {
            return tokenizer.tokenizeToList(input).stream().map(function);
        }

        Stream<String> tokenizeToTransformedMultiset(String input) {
            return tokenizer.tokenizeToMultiset(input).stream().map(function);
        }

        Stream<String> tokenizeToTransformedSet(String input) {
            return tokenizer.tokenizeToSet(input).stream().map(function);
        }
    ...
    static final class Whitespace implements Tokenizer {
     ...
     */
    static class QGram implements Tokenizer {
     ...
     * This class is immutable and thread-safe.
     */
    static class QGramExtended implements Tokenizer {
         ...
         *
         * @param q            size of the tokens
         * @param startPadding padding to apply at the start of short tokens
         * @param endPadding   padding to apply at the end of short tokens
         */
        QGramExtended(int q, String startPadding, String endPadding) {
    ...
    private static <T> Collector<T, Multiset<T>, Multiset<T>> toMultiset() {
        return Collector.of(
                HashMultiset::create,
                Multiset::add,
                (set, other) -> {  set.addAll(other); return set; },
				Function.identity(),
